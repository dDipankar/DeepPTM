...
if __name__ == '__main__':
 ...
 output_H3K9ac = out_label.values[:,5]
 y = np.array(output_H3K9ac, dtype='int8')
 ...
 if os.path.exists(h5filename):
	...
	h5file.create_dataset('/input/H3K9ac_RPKM',data = X_resampled, dtype = np.float64, compression ='gzip')
	#out_group = h5file.get('output')
	h5file.create_dataset('/output/H3K9ac',data = y_resampled, dtype = np.int8, compression ='gzip')	
	
 else:	
	h5file = h5.File(h5filename,'w')
	in_group = h5file.create_group('input')
	in_group.create_dataset('H3K9ac_RPKM',data = X_resampled, dtype = np.float64, compression ='gzip')
	out_group = h5file.create_group('output')
	out_group.create_dataset('H3K9ac',data = y_resampled, dtype = np.int8, compression ='gzip')	 
